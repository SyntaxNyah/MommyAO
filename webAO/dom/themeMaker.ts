...
export interface ThemeConfig {
  ...
  headingFontFamily: string;       // applied to h1-h6, panel titles, character names
  monoFontFamily: string;          // applied to OOC log + code blocks
  displayFontFamily: string;       // applied to the largest titles (modal heading, splash)
  letterSpacing: number;           // -2 to 8 px
  textTransform: "none" | "uppercase" | "lowercase" | "capitalize";
  textShadowStrength: number;      // 0–100 (alpha derived)
  textShadowColor: string;
  textShadowOffsetX: number;       // -10 to 10 px
  textShadowOffsetY: number;       // -10 to 10 px
  textShadowBlur: number;          // 0–20 px
  customFontDataUrl: string;       // uploaded font (data: URL) or ""
  customFontFamilyName: string;    // CSS name to register the uploaded font under
  enableLigatures: boolean;        // enables OpenType liga/dlig
  enableSmallCaps: boolean;        // enables OpenType smcp
...
const DEFAULT_CONFIG: ThemeConfig = {
  ...
  // Typography expansions
  headingFontFamily: "",            // empty = inherit body font
